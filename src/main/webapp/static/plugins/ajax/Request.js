var Request = {
	...
	encodeParam : function(data) {
		...
		for ( var f in data) {
			if (data[f] == "")
				continue;
			if (f.indexOf('$LIKE') != -1 && data[f].indexOf('%') == -1)
				data[f] = "%" + data[f] + "%";
			if (f.indexOf('$START') != -1)
				data[f] = "%" + data[f];
			if (f.indexOf('$END') != -1)
				data[f] = data[f] + "%";
	...
	get : function(uri, data, callback) {
		...
		if (typeof (data) == 'function')
			callback_ = data;
		return Request.doAjax(Request.BASH_PATH + uri, data_, "GET", callback_,
				typeof (callback_) != 'undefined', false);
	},
	post : function(uri, data, callback, requestBody) {
		if (requestBody != false)
			requestBody = true;
		Request.doAjax(Request.BASH_PATH + uri, data, "POST", callback, true,
				requestBody);
	},
	put : function(uri, data, callback, requestBody) {
		if (requestBody != false)
			requestBody = true;
		Request.doAjax(Request.BASH_PATH + uri, data, "PUT", callback, true,
				requestBody);
	},
	patch : function(uri, data, callback, requestBody) {
		if (requestBody != false)
			requestBody = true;
		Request.doAjax(Request.BASH_PATH + uri, data, "PATCH", callback, true,
				requestBody);
	},
	"delete" : function(uri, data, callback) {
		...
		if (typeof (data) == 'function')
			callback_ = data;
		return Request.doAjax(Request.BASH_PATH + uri, data_, "DELETE",
				callback_, typeof (callback_) != 'undefined', false);
	},
	doAjax : function(url, data, method, callback, syc, requestBody) {
		...
		var param = {
			...
			error : function(e) {
				...
				if (msg.code == 401) {
					doLogin(function() {
						Request.doAjax(url, data_tmp, method, callback, syc,
								requestBody);
					});
				} else {
					if (callback)
